...
void ExecutionGraph::dump_as_dot(const std::string& fname,
      const std::vector< std::string >& op_names, 
      size_t node_ofs, size_t max_nodes) {

  std::ofstream ofs(fname + ".dot");
  if (!ofs.is_open()) {
    LOG(WARNING) << "Unable to open file for writing!!";
    return;
  }
  ofs << "digraph dot {\n";

  std::unordered_set< NodeId > seen_nodes, active_nodes;
  auto n_it = nodes_defs_.begin() + node_ofs;
  for (size_t i = 0; n_it != nodes_defs_.end() && i < max_nodes; i++, n_it++) {
    seen_nodes.emplace(n_it->id);
  }
  
  n_it = nodes_defs_.begin() + node_ofs;
  for (size_t i = 0; n_it != nodes_defs_.end() && i < max_nodes; i++, n_it++) {
    for (const auto& e : n_it->out_edges) {
      if (seen_nodes.find(e.id) != seen_nodes.end()) {
        ofs << n_it->id << " -> " << e.id << ";\n";
        active_nodes.emplace(n_it->id);
        active_nodes.emplace(e.id);
      }
    }
  }

  for (size_t i = 0; i < nodes_defs_.size(); i++) {
    auto id = nodes_defs_[i].id;
    if (active_nodes.find(id) == active_nodes.end()) continue;

    auto label = op_names[i];
    ...
    if (label.find("fusion") != std::string::npos ||
      ...
      color = "magenta";
    } else if (label.find("CublasLt") != std::string::npos) {
      color = "orange";
    }
    if (label.length() > 40) {
        label = label.substr(0,38) + "..";
    }
    ofs << id << " [style=bold shape=box color=" << color << " label=\"" 
        << label << "\"];\n";
  }
  ofs << "}\n";
}
